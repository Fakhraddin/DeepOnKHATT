...
def BiRNN_bk(conf_path, batch_x, seq_length, n_input, n_context):
    """
...
def BiRNN(conf_path, batch_x, seq_length, n_input, n_context):
    ...
    outputs = tf.reshape(layer_3, [-1, batch_x_shape[0], n_hidden_3])
    for layer in range(2):
        with tf.variable_scope('lstm_{}'.format(layer),reuse=tf.AUTO_REUSE):
            ...
            # weights, you can do that by giving variable_scope as "lstm" but do
            # make sure first that reuse is set to tf.AUTO_REUSE

            cell_fw = tf.contrib.rnn.LSTMCell(n_cell_dim, initializer=tf.truncated_normal_initializer(-0.1, 0.1, seed=2))
            cell_fw = tf.contrib.rnn.DropoutWrapper(cell_fw, input_keep_prob=1.0 - dropout[4],    output_keep_prob=1.0 - dropout[4])

            cell_bw = tf.contrib.rnn.LSTMCell(n_cell_dim, initializer=tf.truncated_normal_initializer(-0.1, 0.1, seed=2))
            cell_bw = tf.contrib.rnn.DropoutWrapper(cell_bw, input_keep_prob=1.0 - dropout[4],    output_keep_prob=1.0 - dropout[4])
